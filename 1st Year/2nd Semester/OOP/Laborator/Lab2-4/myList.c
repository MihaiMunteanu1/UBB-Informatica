...
Entitate delete(List* l, int poz)
{
    Entitate el = l->medicamente[poz];
    for(int i=poz; i<l->dimensiune-1; i++)
    {
        l->medicamente[i]= l->medicamente[i+1];
    }
    l->dimensiune--;
    return el;
}
...
void destroyUndoList(List* f)
{
    for (int i = 0; i < f->dimensiune; i++)
        destroyList(f->medicamente[i]);
    f->dimensiune=0;
    free(f->medicamente);
    free(f);
