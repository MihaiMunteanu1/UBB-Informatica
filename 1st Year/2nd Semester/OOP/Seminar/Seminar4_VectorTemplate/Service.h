...
private:
	...
	*/
	MyVector<Melodie> filter(function<bool(const Melodie&)> fct);
	...
	*			tip Melodie)
	*/
	MyVector<Melodie> generalSort(bool(*maiMicF)(const Melodie&, const Melodie&));
...
public:
	...
	*/
	...
	const MyVector<Melodie>& getAllSongs() const noexcept {
	...
	*/
	MyVector<Melodie> filtrareGen(string gen);
	...
	*/
	MyVector<Melodie> filtrareDurata(double durataStart, double durataEnd);
	...
	*/
	MyVector<Melodie> sortByArtistSong();

	MyVector<Melodie> sortByArtistNameLength();
