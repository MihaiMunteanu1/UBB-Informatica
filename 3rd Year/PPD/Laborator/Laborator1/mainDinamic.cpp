...
void allocate(const int N,const int M,const int n) {
    F = allocMatrix(N, M);
    C = allocMatrix(n, n);
    V = allocMatrix(N, M);
    V_Secv = allocMatrix(N, M);

}

void deallocate(const int N,const int n) {
    freeMatrix(F, N);
    freeMatrix(C, n);
    freeMatrix(V, N);
    freeMatrix(V_Secv,N);
...
void computeRows(int** F, int** C, int** V, int startRow, int endRow) {
...
void computeColumns(int** F, int** C, int** V, int startCol, int endCol) {
    for (int i = 0; i < N; i++) {
